...
function createLegend(){
  ...
  legend.append("text").text("Number of connections released").attr("x",50).attr("y",115);
  ...
  for ( var i in sizes ){
    legend.append("circle")
      ...
      .attr("vector-effect","non-scaling-stroke");
    legend.append("text")
      .text( (sizes[i] / 1) + " connections" + (i == sizes.length-1 ? "" : "") )
...
function setProbeContent(d){
  ...
  var html = "<strong>" + d.CITY + "</strong><br/>" + " Connections released: " +
            "<strong>" +format( Math.abs( val ) ) + "</strong>"+ ( val < 0 ? "lost" : "" ) + "<br/>" +
            "<span>Number of LPG gas connections released as on 30-05-2018" + "</span>";
  probe
    .html( html );
}
