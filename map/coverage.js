...
function circleSize(d){
  return Math.sqrt( .00003 * Math.abs(d) );
};
...
d3.json("india.json", function(error, us) {
  ...
  d3.csv("coverage.csv",function(data){
    var first = data[0];
    // get columns
...
function createLegend(){
  ...
  legend.append("circle").attr("class","gain").attr("r",6).attr("cx",50).attr("cy",103)
  

  legend.append("text").text("Urban Clusters").attr("x",64).attr("y",108);
  

  var sizes = [ 10000000, 30000000, 70000000];
  for ( var i in sizes ){
    legend.append("circle")
      ...
      .attr("vector-effect","non-scaling-stroke");
    legend.append("text")
      .text( (sizes[i] / 1) + " t CO2" + (i == sizes.length-1 ? "" : "") )
      .attr( "text-anchor", "middle" )
      .attr( "x", 95 + circleSize( sizes[sizes.length-1] ) )
...
function setProbeContent(d){
  ...
  var html = "<strong>" + d.CITY + "</strong><br/>" + " Total CO2 Emission: " +
            "<strong>" +format( Math.abs( val ) ) + "</strong> &nbsptonnes"+ ( val < 0 ? "lost" : "" ) + "<br/>" +
            "<span>For more info visit: http://citycarbonfootprints.info/" + "</span>";
  probe
    .html( html );
}
