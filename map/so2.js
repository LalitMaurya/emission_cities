...
function circleSize(d){
  return Math.sqrt( 2.2 * Math.abs(d) );
};
...
function createLegend(){
  ...
  legend.append("text").text("SO2").attr("x",75).attr("y",83);
...
function setProbeContent(d){
  ...
  var html = "<strong>" + d.CITY + "</strong><br/>" + " SO2 'Annual mean: " +
            "<strong>" +format( Math.abs( val ) ) + "</strong>"+ ( val < 0 ? "lost" : " ug/m3" ) + "<br/>" +
            "<span>Concentration exceeding NAAQS of 50 ug/m3" + "</span>";
  probe
    .html( html );
}
