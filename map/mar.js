...
d3.json("india.json", function(error, us) {
  map.selectAll("path")
      .data(topojson.feature(us, us.objects.worldcountries).features)
      .enter()
      .append("path")
   ...
   map.append("path")
       .datum(topojson.mesh(us, us.objects.worldcountries, function(a, b) { return a !== b; }))
  ...
  d3.csv("mar.csv",function(data){
    ...
    dateScale = createDateScale(orderedColumns).range([0,100]);
...
function createLegend(){
  var legend = g.append("g").attr("id","legend").attr("transform","translate(290,10)");

  legend.append("circle").attr("class","gain").attr("r",7).attr("cx",35).attr("cy",110)
  

  legend.append("text").text("Household using fossil fuels").attr("x",50).attr("y",115);
  

  var sizes = [ 1000000, 5000000, 10000000 ];
  for ( var i in sizes ){
    legend.append("circle")
      .attr( "r", circleSize( sizes[i] ) )
      .attr( "cx", 1 + circleSize( sizes[sizes.length-1] ) )
      .attr( "cy", 2 * circleSize( sizes[sizes.length-1] ) - circleSize( sizes[i] ) )
      .attr("vector-effect","non-scaling-stroke");
    legend.append("text")
      .text( (sizes[i] / 1) + " households" + (i == sizes.length-1 ? "" : "") )
      .attr( "text-anchor", "middle" )
      .attr( "x", 115 + circleSize( sizes[sizes.length-1] ) )
      .attr( "y", 1.5 * ( circleSize( sizes[sizes.length-1] ) - circleSize( sizes[i] ) ) + 3 )
      .attr( "dy", 21)
  }
}
...
function setProbeContent(d){
  ...
  var html = "<strong>" + d.CITY + "</strong><br/>" + " No of households using fossil fuels: " +
            "<strong>" +format( Math.abs( val ) ) + "</strong>"+ ( val < 0 ? "lost" : "" ) + "<br/>" +
            "<span>including (Firewood, Crop residue, Cowdung cake, Coal, Lignite, Charcoal, Kerosene and other)" + "</span>";
  probe
    .html( html );
}
