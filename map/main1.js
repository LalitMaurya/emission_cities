...
function circleSize(d){
  return Math.sqrt( 4.92 * Math.abs(d) );
};
...
function createLegend(){
  var legend = g.append("g").attr("id","legend").attr("transform","translate(600,10)");

  legend.append("circle").attr("class","gain").attr("r",5).attr("cx",55).attr("cy",80)
  legend.append("circle").attr("class","loss").attr("r",5).attr("cx",55).attr("cy",100)

  legend.append("text").text("PM 10").attr("x",75).attr("y",83);
  legend.append("text").text("PM 2.5").attr("x",75).attr("y",103);
  ...
  for ( var i in sizes ){
    legend.append("circle")
      ...
      .attr("vector-effect","non-scaling-stroke");
    legend.append("text")
      .text( (sizes[i] / 1) + "Annual mean, ug/m3" + (i == sizes.length-1 ? "" : "") )
      .attr( "text-anchor", "middle" )
      .attr( "x", 110 + circleSize( sizes[sizes.length-1] ) )
      .attr( "y", 3 * ( circleSize( sizes[sizes.length-1] ) - circleSize( sizes[i] ) ) + 5 )
      .attr( "dy", 5)
  }
}
...
function setProbeContent(d){
  ...
  var html = "<strong>" + d.CITY + "</strong><br/>" +
            format( Math.abs( val ) ) + ", PM 2.5 'Annual mean " + ( val < 0 ? "lost" : " ug/m3" ) + "<br/>" +
